...
def redirect_to_destination(
    ...
    if row["flagged"]:
        raise HTTPException(status.HTTP_403_FORBIDDEN, "this short URL has been flagged and is no longer active")
    ...
    # Only ever cache non-flagged links: flagging always invalidates any
    # existing cache entry (see urls.report_url), so a cache hit here is
    # guaranteed to predate any report -- this is what lets the cache-hit
    # branch above skip the flagged check entirely.
