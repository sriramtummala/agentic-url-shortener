import secrets
from datetime import datetime, timezone
from typing import Optional

from fastapi import APIRouter, Depends, Header, HTTPException, Request, status

from service.app.codegen import CodeGenerationError, generate_code
from service.app.config import DENYLIST_DOMAINS, REPORT_THRESHOLD, SHORT_CODE_LENGTH, SHORT_CODE_MAX_ATTEMPTS
from service.app.db import Database
from service.app.denylist import is_denylisted
from service.app.dependencies import enforce_rate_limit, get_cache, get_db, get_idempotency_store
from service.app.idempotency import IdempotencyStore
from service.app.cache import LRUCache
from service.app.schemas import CreateUrlRequest, CreateUrlResponse, ReportResponse, UrlMetadata

router = APIRouter(prefix="/api/urls", tags=["urls"])


def _is_expired(row) -> bool:
    if row["expires_at"] is None:
        return False
    return datetime.fromisoformat(row["expires_at"]) <= datetime.now(timezone.utc)


@router.post("", response_model=CreateUrlResponse, status_code=status.HTTP_201_CREATED)
def create_url(
    request: Request,
    payload: CreateUrlRequest,
    db: Database = Depends(get_db),
    idempotency_store: IdempotencyStore = Depends(get_idempotency_store),
    idempotency_key: Optional[str] = Header(default=None, alias="Idempotency-Key"),
) -> CreateUrlResponse:
    # Idempotency check must happen before rate limiting, not after: a
    # router-level `dependencies=[Depends(enforce_rate_limit)]` would run on
    # every request regardless of outcome, so a caller safely retrying with
    # the same Idempotency-Key would burn rate-limit budget on every retry
    # and could get 429'd purely from its own retries. Only a genuine
    # cache-miss (new work) should count against the limit.
    if idempotency_key:
        cached = idempotency_store.get(idempotency_key)
        if cached is not None:
            return CreateUrlResponse.model_validate(cached)

    enforce_rate_limit(request)

    destination_url = str(payload.destination_url)
    if is_denylisted(destination_url, DENYLIST_DOMAINS):
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_CONTENT, "destination_url is not allowed")

    try:
        code = generate_code(db.code_exists, length=SHORT_CODE_LENGTH, max_attempts=SHORT_CODE_MAX_ATTEMPTS)
    except CodeGenerationError as exc:
        raise HTTPException(status.HTTP_503_SERVICE_UNAVAILABLE, str(exc)) from exc

    owner_token = secrets.token_urlsafe(24)
    created_at = datetime.now(timezone.utc)

    db.insert_url(
        code=code,
        destination_url=destination_url,
        owner_token=owner_token,
        created_at=created_at.isoformat(),
        expires_at=payload.expires_at.isoformat() if payload.expires_at else None,
    )

    response = CreateUrlResponse(
        code=code, destination_url=destination_url, owner_token=owner_token,
        created_at=created_at, expires_at=payload.expires_at,
    )
    if idempotency_key:
        idempotency_store.put(idempotency_key, response.model_dump(mode="json"))
    return response


@router.get("/{code}", response_model=UrlMetadata)
def get_url_metadata(code: str, db: Database = Depends(get_db)) -> UrlMetadata:
    row = db.get_url(code)
    if row is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "short URL not found")
    if _is_expired(row):
        raise HTTPException(status.HTTP_410_GONE, "short URL has expired")
    return UrlMetadata(
        code=row["code"], destination_url=row["destination_url"],
        created_at=datetime.fromisoformat(row["created_at"]),
        expires_at=datetime.fromisoformat(row["expires_at"]) if row["expires_at"] else None,
        report_count=row["report_count"], flagged=bool(row["flagged"]),
    )


@router.post("/{code}/report", response_model=ReportResponse)
def report_url(
    code: str, db: Database = Depends(get_db), cache: LRUCache = Depends(get_cache),
) -> ReportResponse:
    row = db.get_url(code)
    if row is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "short URL not found")
    report_count, flagged = db.report_url(code, threshold=REPORT_THRESHOLD)
    if flagged:
        # Evict so the next redirect falls through to the DB and sees the
        # flagged state -- a cached hit predates the flag and would
        # otherwise keep redirecting until it aged out on its own.
        cache.invalidate(code)
    return ReportResponse(code=code, report_count=report_count, flagged=flagged)


@router.delete("/{code}", status_code=status.HTTP_204_NO_CONTENT)
def delete_url(
    code: str, x_owner_token: str = Header(...),
    db: Database = Depends(get_db), cache: LRUCache = Depends(get_cache),
) -> None:
    row = db.get_url(code)
    if row is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "short URL not found")
    if not secrets.compare_digest(row["owner_token"], x_owner_token):
        raise HTTPException(status.HTTP_403_FORBIDDEN, "owner token does not match")
    db.delete_url(code)
    cache.invalidate(code)
